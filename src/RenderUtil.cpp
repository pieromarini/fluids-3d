...
void checkShaderError(GLuint shader, bool isProgram, const std::string& errorMessage);
...
GLuint createShader(const std::string& text, GLenum shaderType) {
  ...
  checkShaderError(shader, false, "Error: Program compilation failed: ");
...
void checkShaderError(GLuint shader, bool isProgram, const std::string& errorMessage) {
  ...
  if (success == GL_FALSE) {
	if (isProgram)
	  glGetProgramInfoLog(shader, maxLength, &maxLength, &error[0]);
	else
